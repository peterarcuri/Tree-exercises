...
class BinaryTree {
  ...
  maxSum() {
    ...
    function maxSumHelper(node) { // Helper function
  ...
  nextLarger(lowerBound) {
    ...
    function traverse(node) { // Helper function to perform in-order traversal  
      
      if(!node) return;

      traverse(node.left); // traverse the left subtree first (smaller values)

      if(node.val > lowerBound) { // Process the current node
        if(result === null || node.val < result) {
          result = node.val;
        }
      }
      traverse(node.right); // traverse the right subtree next (larger values)
